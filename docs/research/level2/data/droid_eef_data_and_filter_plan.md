...
- 白名单：单阶段 pick/place/relocate、push/pull/slide、简单 open/close。
- 三条语言标注至少两条命中同一白名单族。
- 任一标注命中以下内容即排除：液体/倾倒、擦拭清洁、布料与袋子、工具操作、电器、插接/拧紧、堆叠、整理、多阶段任务。
- 时长先保留 48–450 frames（3.2–30 秒）；边界后续由分布和人工复核调整。

粗关键词探针得到 30,536 episodes、5,029,773 个 16-step windows，约为官方 filter 后窗口的 36.62%。这只是候选集规模，不能直接作为最终训练 manifest。
...
F1 约 503 万个 16-step windows；若每 episode 封顶 64，理论上限约 195 万，可显著减少一轮训练的重复样本与 I/O。
