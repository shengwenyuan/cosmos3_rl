...
"""RoboLabSim UR5 LeRobot dataset for EEF-space action post-training.

This adapter mirrors the Berkeley UR5 EEF recipe's action contract:

    [se3_delta_translation(3), se3_delta_rot6d(6), gripper(1)]

RoboLabSim stores joint state in ``observation.state`` and EEF pose in separate
``observation.ee_position`` / ``observation.ee_orientation`` fields. The EEF
delta target is therefore built from adjacent observation poses, while the
gripper target is read from the native 7D ``action`` field.
"""
...
_EEF_POS_FEATURE = "observation.ee_position"
_EEF_ORI_FEATURE = "observation.ee_orientation"
_WRIST_VIEW = "observation.images.wrist_cam"
_LEFT_SHOULDER_VIEW = "observation.images.over_shoulder_left_camera"
_RIGHT_SHOULDER_VIEW = "observation.images.over_shoulder_right_camera"
_DEFAULT_CANVAS = (_WRIST_VIEW, _LEFT_SHOULDER_VIEW, _RIGHT_SHOULDER_VIEW)
...
class RoboLabSimUR5EEFDataset(ActionBaseDataset):
    """RoboLabSim UR5 EEF-space dataset with wrist + two shoulder cameras."""
    ...
    def __init__(
        ...
        if viewpoint != "concat_view":
            raise NotImplementedError("RoboLabSimUR5EEFDataset only supports concat_view.")
        self._gripper_invert = bool(gripper_invert)
        super().__init__(
            root=root,
            domain_name="robolabsim-ur5-eef",
        ...
        action_width = _feature_width(self._info, _ACTION_FEATURE)
        if action_width != _NATIVE_ACTION_DIM:
            raise ValueError(
                f"RoboLabSim UR5 EEF adapter expects `{_ACTION_FEATURE}` width {_NATIVE_ACTION_DIM}, "
                f"got {action_width}."
            )
        pos_width = _feature_width(self._info, _EEF_POS_FEATURE)
        ori_width = _feature_width(self._info, _EEF_ORI_FEATURE)
        if pos_width != _EEF_POS_DIM:
            raise ValueError(f"Expected `{_EEF_POS_FEATURE}` width {_EEF_POS_DIM}, got {pos_width}.")
        if ori_width != _EEF_QUAT_DIM:
            raise ValueError(f"Expected `{_EEF_ORI_FEATURE}` width {_EEF_QUAT_DIM}, got {ori_width}.")
    ...
    def _stats_path(cls) -> Path:
        return Path(__file__).parent / "stats/robolabsim_ur5_eef_stats.json"
    ...
    def _resolve_canvas(self, canvas_views: tuple[str, ...] | None) -> list[str]:
        ...
        if canvas_views is None:
            feats = list(_DEFAULT_CANVAS)
        else:
        ...
        if not feats:
            raise ValueError(f"No RoboLabSim observation.images.* cameras found; have {sorted(available)}.")
        return feats[:3]
    ...
    def _build_eef_action(
        ...
        pos = np.asarray([_row_vector(row, _EEF_POS_FEATURE) for row in observation_rows], dtype=np.float32)
        ori = np.asarray([_row_vector(row, _EEF_ORI_FEATURE) for row in observation_rows], dtype=np.float32)
        if pos.shape[-1] != _EEF_POS_DIM:
            raise ValueError(f"Expected RoboLabSim EEF position width {_EEF_POS_DIM}, got {pos.shape[-1]}.")
        if ori.shape[-1] != _EEF_QUAT_DIM:
            raise ValueError(f"Expected RoboLabSim EEF orientation width {_EEF_QUAT_DIM}, got {ori.shape[-1]}.")

        poses_abs = build_abs_pose_from_components(pos, ori, "quat_xyzw")
        ...
        if native.shape[-1] != _NATIVE_ACTION_DIM:
            raise ValueError(f"Expected RoboLabSim action width {_NATIVE_ACTION_DIM}, got {native.shape[-1]}.")
...
def get_action_robolabsim_ur5_eef_sft_dataset(
    *,
    root: str,
    fps: float | None = None,
    chunk_length: int = 32,
    ...
    """Build the RoboLabSim UR5 EEF-space action SFT dataset."""
    dataset = RoboLabSimUR5EEFDataset(
        root=root,
        fps=fps,
        chunk_length=chunk_length,
        mode=mode,
