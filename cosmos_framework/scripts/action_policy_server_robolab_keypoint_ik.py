...
"""WebSocket inference server for RoboLab keypoint-IK Action policy serving.
...
This divided entry point keeps the original serving logic but exposes only two
action-space modes: ``joint_pos`` and ``eef_pose``.  Their raw widths are
configured through representation-specific arguments instead of being tied to a
single Franka-only shape.

Example:

  PYTHONPATH=. python -m cosmos_framework.scripts.action_policy_server_robolab_keypoint_ik \
    --checkpoint-path <checkpoint>/model \
    --domain-name robolabsim-ur5-eef \
...
_CONCAT_VIEW_DESCRIPTION = (
    ...
    "The bottom-left row is the left shoulder camera. "
    "The bottom-right row is the right shoulder camera."
)
...
ActionSpace = Literal["joint_pos", "eef_pose"]
...
def _load_training_config(setup_args: OmniSetupArgs, checkpoint_path: str) -> Any | None:
    ...
    try:
        ...
        config_file = getattr(setup_args, "config_file", None)
        if config_file is not None:
            try:
                return deserialize_config_dict(Path(config_file))
            except Exception as fallback_exc:
                log.warning(
                    "[robolab-policy-server] could not load training config for transforms: "
                    f"{exc}; raw config fallback failed: {fallback_exc}"
                )
                return None
...
class RobolabPolicyConfig:
    ...
    joint_dof: int = _DEFAULT_JOINT_DOF
    gripper_dim: int = _DEFAULT_GRIPPER_DIM
...
class RobolabServerArgs(pydantic.BaseModel):
    ...
    """Raw action dimension. If omitted, derives from action-space representation arguments."""
    joint_dof: int = _DEFAULT_JOINT_DOF
    """Number of joint-position channels before gripper channels for joint_pos."""
    gripper_dim: int = _DEFAULT_GRIPPER_DIM
    """Number of gripper channels appended to the action vector. Current execution supports 1."""
    image_height: int = _DEFAULT_IMAGE_HEIGHT
    ...
    action_space: ActionSpace = "joint_pos"
    """RoboLab action representation: joint_pos or eef_pose."""
    use_state: bool = True
...
def _resolve_action_dim(args: RobolabServerArgs) -> int:
    if args.joint_dof <= 0:
        raise ValueError("--joint-dof must be positive")
    if args.gripper_dim <= 0:
        raise ValueError("--gripper-dim must be positive")
    ...
    if args.action_space == "joint_pos":
        expected_dim = int(args.joint_dof) + int(args.gripper_dim)
        if int(args.gripper_dim) != 1:
            raise NotImplementedError(
                "--gripper-dim != 1 is reserved; current joint_pos execution reads one gripper channel"
            )
    elif args.action_space == "eef_pose":
        expected_dim = _DEFAULT_EEF_POSE_ACTION_DIM
    else:
        raise ValueError(f"Unsupported action_space={args.action_space!r}")

    resolved_dim = int(args.action_dim) if args.action_dim is not None else expected_dim
    if resolved_dim != expected_dim:
        if args.action_space == "eef_pose":
            raise NotImplementedError(
                f"--action-dim={resolved_dim} for eef_pose is reserved; "
                f"current eef_pose execution supports only {expected_dim}D"
            )
        raise ValueError(
            f"--action-dim={resolved_dim} is inconsistent with --action-space={args.action_space!r}; "
            f"expected {expected_dim} from --joint-dof and --gripper-dim"
        )
    return resolved_dim
...
class RobolabPolicyService:
    def __init__(self, args: RobolabServerArgs) -> None:
        ...
        self.cfg = RobolabPolicyConfig(
            ...
            action_dim=_resolve_action_dim(args),
            image_height=int(args.image_height),
            image_width=int(args.image_width),
            action_space=args.action_space,
            joint_dof=int(args.joint_dof),
    ...
    def _build_transform(self, training_config: Any | None, args: RobolabServerArgs) -> tuple[Any, dict[str, Any]]:
        ...
        if isinstance(training_config, dict):
            dataset_groups = (
                training_config.get("dataloader_train", {}).get("dataloader", {}).get("datasets", {})
            )
            action_dataset_config = None
            if isinstance(dataset_groups, dict) and dataset_groups:
                first_dataset = next(iter(dataset_groups.values()))
                if isinstance(first_dataset, dict):
                    action_dataset_config = first_dataset.get("dataset")
            if isinstance(action_dataset_config, dict):
                chunk_length = action_dataset_config.get("chunk_length")
                if isinstance(chunk_length, int):
                    inferred["action_chunk_size"] = chunk_length
                fps = action_dataset_config.get("fps")
                if isinstance(fps, (int, float)):
                    inferred["conditioning_fps"] = float(fps)
                if args.resolution is None and action_dataset_config.get("resolution") is not None:
                    inferred["resolution"] = str(action_dataset_config["resolution"])
                return (
                    ActionTransformPipeline(
                        max_action_dim=max_action_dim,
                        cfg_dropout_rate=0.0,
                        append_viewpoint_info=bool(action_dataset_config.get("append_viewpoint_info", True)),
                        append_duration_fps_timestamps=bool(
                            action_dataset_config.get("append_duration_fps_timestamps", True)
                        ),
                        append_resolution_info=bool(action_dataset_config.get("append_resolution_info", True)),
                        append_idle_frames=bool(action_dataset_config.get("append_idle_frames", False)),
                    ),
                    inferred,
                )
