...
from cosmos_framework.inference2.arch_mapping import read_model_arch
...
def convert_model_to_diffusers(args: Args) -> None:
    ...
    # The reversible arch kwargs are sourced from ARCH_MAP (inference2.arch_mapping), the single
    # list of flat model.config knobs written into transformer/config.json; a lockstep test proves
    # it covers every such converter arg. The LM-text (lm_cfg.*) and structural/derived fields
    # below are read directly from the net.
    _arch = read_model_arch(unstructure_config(model_cfg))
    ...
    latent_patch_size = _arch["latent_patch_size"]
    latent_channel = _arch["latent_channel"]
    timestep_scale = _tmp.net.timestep_scale
    base_fps = int(_arch["base_fps"])
    enable_fps_modulation = _arch["enable_fps_modulation"]
    max_action_dim = _tmp.config.max_action_dim
    unified_3d_mrope_reset_spatial_ids = _arch["unified_3d_mrope_reset_spatial_ids"]
    unified_3d_mrope_temporal_modality_margin = _arch["unified_3d_mrope_temporal_modality_margin"]
    ...
    # Modality flags/dims: primary value from the shared map (_arch), OR'd with the
    # weight-presence fallback the converter has always applied for robustness.
    action_gen = bool(_arch.get("action_gen", False) or has_action_projection_weights)
    action_dim = _arch.get("action_dim")
    if action_dim is None and action_proj_in is not None:
        action_dim = getattr(action_proj_in, "input_size", None)
    if action_dim is None:
        action_dim = max_action_dim
    num_embodiment_domains = int(_arch.get("num_embodiment_domains", 32))
    ...
    sound_gen = bool(_arch.get("sound_gen", False) or has_sound_projection_weights)
    sound_dim = _arch.get("sound_dim")
    if sound_dim is None and sound2llm is not None:
        sound_dim = sound2llm.in_features
    sound_latent_fps = _arch.get("sound_latent_fps", 25.0)
    if sound_gen:
