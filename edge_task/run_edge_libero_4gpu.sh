...
export NPROC_PER_NODE=4 PYTHONUNBUFFERED=1
...
[[ "$GPU_COUNT" == 4 ]] || {
    echo "ERROR: Cosmos training requires 4 visible GPUs; found $GPU_COUNT" >&2
    exit 1
}

if [[ "${EDGE_LIBERO_MODE:-train}" == smoke ]]; then
    exec env EXTRA_TAIL_OVERRIDES="trainer.max_iter=5 checkpoint.save_iter=999999 checkpoint.save_last_checkpoint=false job.group=smoke job.name=action_policy_libero_all_edge_4gpu_smoke job.wandb_mode=disabled" \
...
exec bash examples/launch_sft_action_policy_libero_all_edge_4gpu.sh
