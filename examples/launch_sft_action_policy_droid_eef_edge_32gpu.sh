...
# Four-node, 32-GPU Cosmos3-Edge DROID Fx4 EEF Stage-1 training.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TOML_FILE="$SCRIPT_DIR/toml/sft_config/action_policy_droid_eef_edge_32gpu.toml"
...
[[ "$NODE_RANK" =~ ^[0-3]$ ]] || { echo "ERROR: NODE_RANK must be 0, 1, 2, or 3." >&2; exit 1; }

export ACTION_STATS_PATH BASE_CHECKPOINT_PATH DROID_ROOT EDGE_BASE_PATH FX4_TRAIN_MANIFEST
export MASTER_ADDR NNODES NODE_RANK NPROC_PER_NODE OUTPUT_ROOT PYTHON_BIN WAN_VAE_PATH
export LD_LIBRARY_PATH=""
DATASET_PATH="$DROID_ROOT"
LOG_FILENAME="action_policy_droid_eef_edge_32gpu_rank${NODE_RANK}.log"
