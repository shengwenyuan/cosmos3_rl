#!/usr/bin/env bash
# Volcengine / multi-H20 orchestration wrapper for RoboLabSim UR5 EEF SFT.
#
# This is the outer, cloud-job-facing entry point for the small RoboLabSim-147
# probe. It validates mounted datasets/checkpoints, sources the dev-image
...
DP_SHARD="${DP_SHARD:-8}"
RECOMMENDED_WORLD_SIZE="${RECOMMENDED_WORLD_SIZE:-8}"
...
validate_static_env_values() {
  local name
  for name in JOB_NAME NPROC_PER_NODE NNODES NODE_RANK MASTER_ADDR MASTER_PORT DATASET_PATH ROBOLABSIM_UR5_ROOT BASE_CHECKPOINT_PATH WAN_VAE_PATH; do
...
resolve_topology() {
  ...
  export NPROC_PER_NODE NNODES NODE_RANK MASTER_PORT WORLD_SIZE DP_SHARD DP_REPLICATE MAX_SAMPLES_PER_BATCH
...
validate_paths() {
  ...
  require_dir "$ROBOLABSIM_UR5_ROOT"
  require_file "$ROBOLABSIM_UR5_ROOT/meta/info.json"
  require_file "$ROBOLABSIM_UR5_ROOT/meta/tasks.parquet"
  require_file "$ROBOLABSIM_UR5_ROOT/meta/cameras.json"
  require_dir "$ROBOLABSIM_UR5_ROOT/data"
  require_dir "$ROBOLABSIM_UR5_ROOT/videos"
...
validate_dataset_metadata() {
  python - "$ROBOLABSIM_UR5_ROOT/meta/info.json" <<'PY_DATASET'
import json
import sys
...
if errors:
    print("RoboLabSim metadata validation failed:", file=sys.stderr)
    for err in errors:
        print(f"  - {err}", file=sys.stderr)
    raise SystemExit(1)
print(
    "RoboLabSim metadata ok:",
...
print(
    ...
    "top=wrist_cam bottom-left=over_shoulder_left_camera bottom-right=over_shoulder_right_camera",
)
PY_DATASET
}
...
log_effective_plan() {
  ...
  log "  MAX_ITER=$MAX_ITER SAVE_ITER=$SAVE_ITER LOGGING_ITER=$LOGGING_ITER OPTIMIZER_LR=$OPTIMIZER_LR SCHEDULER_CYCLE_LENGTH=$SCHEDULER_CYCLE_LENGTH"
...
launch_training() {
  ...
  log "Launching RoboLabSim UR5 EEF SFT."
  log "JOB_NAME=$JOB_NAME"
...
main() {
  ...
  log "Starting Volcengine RoboLabSim UR5 EEF orchestration."
  log_platform_env_summary
  validate_paths
  validate_dataset_metadata
  validate_hardware
